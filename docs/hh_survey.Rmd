...
summary_tbl <- nm_tours %>%
  ...
  group_by(HOUSEID) %>%
  summarize(
    joint_tour = sum(joint_tour),
    indiv_tour = sum(indiv_tour)
  ) 
nm_pattern_choice <- hh_with_structure %>%
  select(HOUSEID, weight = WTHHFIN5D_HI) %>%
  left_join(summary_tbl, by = "HOUSEID") %>%
  replace_na(list("joint_tour" = 0, "indiv_tour" = 0)) %>%
  mutate(pattern = case_when(
    joint_tour + indiv_tour == 0 ~ "N",
    joint_tour >= 1 & indiv_tour == 0 ~ "J",
    joint_tour == 0 & indiv_tour >= 1 ~ "I",
    TRUE ~ "IJ"
  )) %>%
  group_by(pattern) %>%
  summarize(weight = sum(weight, na.rm = TRUE)) %>%
