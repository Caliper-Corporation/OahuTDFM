...
# ID Subtours
When someone on a work tour leaves work and returns to work
...
# tour people who traveled together) must be removed.
...
joint_comp <- trip_add_hh_tour_id %>%
  ...
  slice(1) %>%
  ungroup() %>%
  select(HOUSEID, hh_tour_id, ONTD_P1:ONTD_P15) %>%
  ...
  group_by(HOUSEID, hh_tour_id, type) %>%
  summarize(count = n()) %>%
  ungroup() %>%
  pivot_wider(
    id_cols = HOUSEID:hh_tour_id, names_from = type, values_from = count,
    values_fill = 0
  ) %>%
  ungroup() %>%
  filter(A > 0) %>%
  ...
  # # ignore the 
  # filter(Alternative != "A1K0") %>%
  group_by(Alternative) %>%
  summarize(weight = sum(weight, na.rm = TRUE)) %>%
  mutate(target_share = round(weight / sum(weight), 2))
```
...
write_csv(joint_comp, "data/_private/hh_survey/output/calibration_targets/joint_composition.csv", na = "")
